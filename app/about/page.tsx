...
<section><h2>콘텐츠와 이미지 원칙</h2><p>퍼즐리의 사용 안내, 난이도 설명, 퍼즐 팁은 서비스 운영팀이 직접 작성합니다. 추천 이미지는 Pexels 검색 API를 통해 불러오며 각 카드에 사진작가와 제공처 출처를 표시합니다. API 요청 한도, 인증 또는 네트워크 문제로 실시간 검색을 완료하지 못하면 오류 원인을 안내하고 사전 선별한 Unsplash 이미지 10장을 대신 제공합니다. 퍼즐리는 외부 이미지의 소유권을 주장하지 않으며 권리 관련 요청은 확인 후 대응합니다.</p><p>어린이부터 성인까지 부담 없이 사용할 수 있는 화면과 표현을 지향하며, 과도한 광고나 퍼즐 진행을 방해하는 배치를 피하는 것을 운영 원칙으로 삼습니다.</p></section>
<aside className="info-cta"><h2>직접 퍼즐을 만들어보세요</h2><p>단어 하나로 열 장의 후보를 살펴보고 12피스부터 400피스까지 난이도를 골라볼 수 있습니다.</p><Link href="/#make">퍼즐 만들기 →</Link></aside></InfoPage>}
